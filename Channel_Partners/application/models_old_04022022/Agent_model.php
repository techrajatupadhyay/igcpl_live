...
class agent_model extends CI_Model{
    ...
    public function GetAllLaabhagent($user_type,$user_id,$regionid,$region_state,$district_branch)
    {
		...
		else if($user_type==6)
		{
			...
			$sql = "SELECT * FROM `laabh_agents` WHERE  region='".$regionid."' and region_state='".$region_state."' and district_branch='".$district_branch."' and status='ACTIVE' ";
			$query = $this->db->query($sql);
			$result = $query->result();
			//print_r($this->db->last_query());
    ...
    public function laabhagentSingle($user_type,$user_id,$regionid)
    {
        ...
        if($user_type==1)
		{     
			$sql = "SELECT * FROM laabh_agents WHERE  status='ACTIVE' limit 1";
