...
{   
    ...
    public function add_vendor($data, $client_id = null)
 ...
 public function get_order_tax_by_id($id){
    ...
    public function add_pur_request($data){
        ...
        if(isset($data['request_detail'])){
            ...
            $request_detail = explode (',', $data['request_detail']);
            ...
            foreach($rq_val as $key => $rq){
                ...
                if($rq['item_code'] != ''){
                    ...
                    array_push($rq_detail, $rq_val[$key]);
    ...
    {   
        ...
        if(isset($data['estimate_detail'])){
            ...
            $estimate_detail = explode (',', $data['estimate_detail']);
            ...
            for ($i=0; $i < count($estimate_detail); $i++) {

                

                    $row[] = $estimate_detail[$i];
        ...
        if ($insert_id) {
            ...
            update_sales_total_tax_column($insert_id, 'estimate', db_prefix() . 'pur_estimates');
    ...
    $html = '<table class="table">
        ...
        <tbody>
          ...
          <tr class="border_tr">
            ...
            <td class="border_td">'._l('quantity').'</td>
             ...
             <td class="border_td">'._l('total_tax').'</td>
      ...
      foreach($pur_order_detail as $row){
        ...
        $taxes = $this->get_order_tax_by_id($row['pur_order']);

        $html .= '<tr class="border_tr">

            <td class="border_td_left">'.$row['item_code'].'</td>
            ...
            <td class="border_td_right">'.$row['quantity'].'</td>
             ...
             <td class="border_td_right">'.app_format_money($taxes->total_tax,'').'</td>
      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr>';
            ...
            $html .= '<td></td><td class="td_ali_font"><h3>'.mb_strtoupper(_l('order')).'</h3></td>
      ...
      $html .= '<br> <br>
      <span><b>Terms & Conditions </b> <br> '. $pur_order->terms . ' </span>
      <link href="' . module_dir_url(PURCHASE_MODULE_NAME, 'assets/css/pur_order_pdf.css') . '"  rel="stylesheet" type="text/css" />';
