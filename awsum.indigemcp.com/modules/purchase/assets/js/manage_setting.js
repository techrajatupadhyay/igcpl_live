(function($) {
  ...
  $('.account-template-form-submiter').on('click', function() {
    ...
    function edit_approval_setting(invoker,id){
      ...
      $.post(admin_url + 'accounting/get_html_approval_setting/'+ id).done(function(response) {
   ...
   function new_approval_setting(){
      ...
      $.post(admin_url + 'accounting/get_html_approval_setting').done(function(response) {
         response = JSON.parse(response);
