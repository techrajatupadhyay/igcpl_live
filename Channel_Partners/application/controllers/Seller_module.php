...
class Seller_module extends CI_Controller 
{
	...
	die;
		if($this->input->post('isedit')==0)
		{
			...
			if($count<=0)
			{
	            ...
	            $fileName = basename($_FILES['eway_bill_part_1']['name']);
	            ...
	            {    
	                ...
	                move_uploaded_file($_FILES["eway_bill_part_1"]["tmp_name"], $targetFilePath2); 
	            }
				...
				$saveData=array(
						...
						'eway_bill_part_1'=> $targetFilePath2,
	    ...
	    else
	    {
            ...
            if($_FILES['gem_workorder_doc']['file'] !=null)				
            {
                ...
                return redirect("Seller_module/Workorder_Payment/".$sellerid."/".$igcpl_workorder_id." ");
    ...
    public function Workorder_Payment($sellerid)
	{
		...
		if ($this->session->userdata('user_login_access') == 1)
		{
			...
			$data['workorder_payment'] = $this->db->query($sql)->result();
			...
			$this->load->view('backend/new_header'); 
			$this->load->view('backend/new_sidebar'); 
			$this->load->view('backend/workorder_payment', $data);
			$this->load->view('backend/new_footer');
