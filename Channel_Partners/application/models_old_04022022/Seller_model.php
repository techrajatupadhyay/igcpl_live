...
public function sendSMS($mobile,$message){
    ...
    public function GetAllSeller($user_type,$user_id,$region,$region_state,$district_branch)
    {
		...
		else if($user_type==6)
		{
			...
			$sql = "SELECT * FROM `seller` WHERE region_id='".$region."' AND region_state='".$region_state."' AND district_branch='".$district_branch."' AND isactive=1";
			$query = $this->db->query($sql);
			$result = $query->result();
			//print_r($this->db->last_query());
    ...
    public function get_my_workorders($regionid,$usertype,$user_id,$region_state,$district_branch)
    {
		...
		{
			/*
				$sql ="SELECT * from seller Where executive_id=".$user_id." and region_id=".$regionid." and isactive='0' and status='1' " ;
				$data['seller'] = $this->db->query($sql)->result();
		...
		else if($usertype==6)
		{
			...
			$sql = "SELECT * FROM `work_order` WHERE region_id=".$regionid." and isactive='0' and region_state='".$region_state."' and district_branch='".$district_branch."' and status='1' ";
			$query = $this->db->query($sql);
			$result = $query->result();
			//print_r($this->db->last_query());
